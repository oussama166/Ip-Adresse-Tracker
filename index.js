var ipGet = document.getElementById("ip-address");
...
var confiButton = document.querySelector("button");

confiButton.addEventListener("click",(e)=>{
  main(ipGet.value);
  e.preventDefault();
})
var mymap = L.map("map").setView([34.0614, -118.08162], 15);
L.tileLayer(
...
function main(ipAddress) {
  var ip = ipAddress;
  var api_key = "at_OE3YvXGw5hcgbO4wPnbzd8WLBlvu0&ipAddress";
...
function displayMap(res) {
  mymap.setView([res.location.lat, res.location.lng], 15);
